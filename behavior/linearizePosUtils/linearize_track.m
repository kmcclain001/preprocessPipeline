...
function [f, G, fig] = linearize_track(filename,plot_info)
    ...
    x_all = vertcat(x{:});
    y_all = vertcat(y{:});
    outlierInds = abs(x_all-mean(x_all))>(2*std(x_all))|abs(y_all-mean(y_all))>(1.5*std(y_all));
    ...
    %c = occ.^2;
    thresh = prctile(occ(occ>0),[25,99]);
    occ_rect = occ;
    ...
    % Smooth Heat Map
    occ_smooth = imgaussfilt(occ_bord,floor(border/8));
