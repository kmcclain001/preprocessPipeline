function ops = KilosortConfiguration_km(varargin)
%K% edited 3/11/21
...
templatemultiplier = 9; % 9 times more templates created than Nchan
%km%ops.Nfilt = ops.Nchan*templatemultiplier - mod(ops.Nchan*templatemultiplier,32); % number of filters to use (2-4 times more than Nchan, should be a multiple of 32)
...
%km%ops.NT               = 32*1028+ ops.ntbuff;% this is the batch size (try decreasing if out of memory) for GPU should be multiple of 32  + ntbuff
%km%ops.NT               =  1*32*1028+ ops.ntbuff;% 
ops.NT               =  1*32*100+ ops.ntbuff;
...
ops.lam              = [15 50 50];
%km%ops.lam              = [12 40 40];   % large means amplitudes are forced around the mean ([10 30 30])
...
%km%ops.initialize      = 'fromData';    %'fromData' or 'no'
ops.spkTh           = -3;
%km%ops.spkTh           = -4;      % spike threshold in standard deviations (4)
