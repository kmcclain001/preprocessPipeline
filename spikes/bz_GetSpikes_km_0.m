...
ksDirs = dir([basepath filesep '*kilosort*']);
nPaths = length(ksDirs);
...
for pIdx = 1:nPaths
    kilosort_path = [ksDirs(pIdx).folder filesep ksDirs(pIdx).name];
    disp(['loading spikes from Kilosort/Phy format...' num2str(pIdx) '/' num2str(nPaths)]);
    ...
    for ii = 1:length(cluster_group.group)
        ...
        if strcmpi(strtrim(cluster_group.group(ii,:)),'good')
            ...
            spikes.cluID(cellCount) = cluster_group.cluster_id(ii);
            spikes.ksPath(cellCount) = pIdx;
            spikes.UID(cellCount) = cellCount;
            spikes.times{cellCount} = double(spike_times(ids))/fs; % cluster time
            spikes.ts{cellCount} = double(spike_times(ids)); % cluster time
